...
class MangaLivreSource:
    ...
    async def search(self, query: str, limit: int = 10) -> list[MangaSearchResult]:
        ...
        response = await get_client().get(url, params=params, headers=BROWSER_HEADERS)
    ...
    async def detail(self, manga_source_id: str) -> MangaSearchResult:
        ...
        response = await get_client().get(url, headers=BROWSER_HEADERS)
    ...
    async def chapters(self, manga_source_id: str, language: str = "pt-br") -> list[Chapter]:
        ...
        pattern = rf'<a href="(https://mangalivre\.to/manga/{slug}/(capitulo-([^/"]+))/?)"[^>]*>\s*(.*?)\s*</a>'
...
def _extract_genres(text: str) -> list[str]:
    """Le os generos do bloco `genres-content` da pagina do manga."""
    block = re.search(
        r'class="genres-content">(.*?)</div>', text, re.DOTALL
    )
    if not block:
        return []
